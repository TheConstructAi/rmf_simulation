...
static double compute_yaw(const Eigen::Isometry3d &pose,
  ...
  double eps = 0.01;
  // Account for edge cases where the robot rotates past -PI rad to PI rad, or
  // vice versa
...
double compute_friction_energy(const double f, const double m, const double v,
                               const double dt) {
...
line_circle_intersections(const Eigen::Vector2d point1,
                          const Eigen::Vector2d point2, const double cx,
                          const double cy, const double radius) {
  ...
  // From the equations for the line segment from point1 to point2 with
  // parameter t, and the circle, form the quadratic equation for t. Then A, B
  // and C are the coefficients of the quadratic equation.
  A = dx * dx + dy * dy;
  B = 2 * (dx * (point1(0) - cx) + dy * (point1(1) - cy));
  C = (point1(0) - cx) * (point1(0) - cx) +
      (point1(1) - cy) * (point1(1) - cy) - radius * radius;
  ...
  if ((A <= 1e-7) || (det < 0)) {
    ...
    intersections.push_back(
        Eigen::Vector2d(point1(0) + t * dx, point1(1) + t * dy));
  } else {
    // Two solutions.
...
// closest to point P.
static Eigen::Vector2d get_closest_point_on_line_segment(Eigen::Vector2d A,
                                                         Eigen::Vector2d B,
                                                         Eigen::Vector2d P) {
...
std::string SlotcarCommon::model_name() const { return _model_name; }
...
bool SlotcarCommon::path_request_valid(
  ...
  // Empty task request
  if (msg->path.size() == 0) {
    RCLCPP_WARN(logger(), "[%s] received a path [%s] with no waypoints",
                _model_name.c_str(), msg->task_id.c_str());
    return false;
  }
  return true;
}
...
void SlotcarCommon::path_request_cb(
  ...
  RCLCPP_DEBUG(logger(), "[%s] received a path request [%s] with %d waypoints",
               _model_name.c_str(), msg->task_id.c_str(),
               (int)msg->path.size());
  ...
  for (size_t i = 0; i < msg->path.size(); ++i) {
    Eigen::Vector3d v3(msg->path[i].x, msg->path[i].y, 0);

    Eigen::Vector3d yaw_euler(0, 0, msg->path[i].yaw);
...
std::array<double, 2> SlotcarCommon::calculate_control_signals(
  ...
  rmf_plugins_utils::MotionParams drive_params{
      max_lin_vel, _max_drive_acceleration, _nominal_drive_acceleration, 0.01,
      10000000.0};
  const double v_target = rmf_plugins_utils::compute_desired_rate_of_change(
      displacements.first, v_robot, linear_speed_target_now,
      linear_speed_target_destination, drive_params, dt);

  rmf_plugins_utils::MotionParams turn_params{
      _nominal_turn_speed, _max_turn_acceleration, _nominal_turn_acceleration,
      0.01, 10000000.0};
  const double w_target = rmf_plugins_utils::compute_desired_rate_of_change(
      displacements.second, w_robot, _nominal_turn_speed, 0.0, turn_params, dt);
...
std::array<double, 2> SlotcarCommon::calculate_joint_control_signals(
  ...
  std::array<double, 2> new_velocities = calculate_control_signals(
      ...
      linear_speed_target_destination, linear_speed_limit);

  std::array<double, 2> joint_signals;
  for (std::size_t i = 0; i < 2; ++i) {
    const double yaw_sign = i == 0 ? -1.0 : 1.0;
    joint_signals[i] =
        (new_velocities[0] / _tire_radius) +
        (yaw_sign * new_velocities[1] * _base_width / (2.0 * _tire_radius));
  }
...
SlotcarCommon::UpdateResult
SlotcarCommon::update(const Eigen::Isometry3d &pose,
...
SlotcarCommon::UpdateResult SlotcarCommon::update_diff_drive(
    const std::vector<Eigen::Vector3d> &obstacle_positions, const double time) {
  UpdateResult result;
  ...
  if (_initialized_pose) {
    const Eigen::Vector3d dist =
        compute_dpos(_old_pose, _pose); // Ignore movement along z-axis
  ...
  if (_traj_wp_idx < trajectory.size()) {
    ...
    const Eigen::Vector3d dpos =
        compute_dpos(trajectory.at(_traj_wp_idx).pose, _pose);
    ...
    if (rotate_towards_next_target) {
      if (_traj_wp_idx + 1 < trajectory.size()) {
        ...
        double dir = 1.0;
        result.w = compute_change_in_rotation(current_heading, dpos_next,
                                              &goal_heading, &dir);
      ...
      _remaining_path.erase(_remaining_path.begin());
      RCLCPP_DEBUG(logger(), "%s reached waypoint %ld/%d", _model_name.c_str(),
                   _traj_wp_idx, (int)trajectory.size());
      if (_traj_wp_idx == trajectory.size()) {
        RCLCPP_DEBUG(logger(), "%s reached goal -- rotating to face target",
                     _model_name.c_str());
      }
    }
    ...
    if (!rotate_towards_next_target && _traj_wp_idx < trajectory.size()) {
      ...
      result.v = std::abs(result.w) < d_yaw_tolerance ? dir * dpos_mag : 0.0;
      if (result.v != 0.0) {
  ...
  if (_traj_wp_idx < trajectory.size()) {
    ...
    if (dpos_mag < close_enough_threshold) {
      ...
      RCLCPP_DEBUG(logger(), "%s reached waypoint %ld/%d", _model_name.c_str(),
                   _traj_wp_idx, (int)trajectory.size());
      if (_traj_wp_idx == trajectory.size()) {
        RCLCPP_DEBUG(logger(), "%s reached goal", _model_name.c_str());
      } else {
    ...
    if (_traj_wp_idx < trajectory.size()) {
      ...
      if ((_traj_wp_idx == trajectory.size() - 1) &&
        ...
        // points A and B represent the start and end of the current path
        // segment.
        ...
        if (_traj_wp_idx == 0) {
          ...
          point_A =
              trajectory.at(_traj_wp_idx - 1).pose.translation().head<2>();
        }
        ...
        auto intersections = line_circle_intersections(
            point_A, point_B, _pose.translation()(0), _pose.translation()(1),
            _lookahead_distance);
      ...
      _lookahead_point =
          Eigen::Vector3d(target(0), target(1),
      ...
      auto goal_heading = compute_heading(trajectory.at(_traj_wp_idx).pose);
      double dir = 1.0;
      result.w = compute_change_in_rotation(current_heading, d_target,
                                            &goal_heading, &dir);
...
double SlotcarCommon::compute_change_in_rotation(
    const Eigen::Vector3d &heading_vec, const Eigen::Vector3d &dpos,
    const Eigen::Vector3d *traj_vec, double *const dir) const {
  if (dpos.norm() < 1e-3) {
...
void SlotcarCommon::charge_state_cb(const std::string &name, bool selected) {
...
bool SlotcarCommon::near_charger(const Eigen::Isometry3d &pose) const {
  ...
  if (waypoints_it != _charger_waypoints.end()) {
    ...
    for (const ChargerWaypoint &waypoint : waypoints) {
      ...
      double dist = sqrt(pow(waypoint.x - pose.translation()[0], 2) +
...
double SlotcarCommon::compute_discharge(const Eigen::Vector3d lin_vel,
                                        const double ang_vel,
                                        const Eigen::Vector3d lin_acc,
                                        const double ang_acc,
  ...
  const double EA =
      ((_params.mass * a * v) + (_params.inertia * alpha * w)) * run_time;
...
std::string SlotcarCommon::get_left_wheel_joint() const {
  return _left_wheel_joint_name;
}
