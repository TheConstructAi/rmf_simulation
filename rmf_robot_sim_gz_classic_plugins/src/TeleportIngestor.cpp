...
private:
  // Stores params representing state of Ingestor, and handles all message
  // pub/sub
...
SimEntity
TeleportIngestorPlugin::find_nearest_model(const std::vector<SimEntity> &models,
                                           bool &found) const {
...
#else
  const ignition::math::AxisAlignedBox vicinity_box(robot_collision_bb.Min(),
                                                    max_corner);
#endif
...
void TeleportIngestorPlugin::send_ingested_item_home() {
  if (_ingestor_common->ingestor_filled) {
    ...
    if (it == _ingestor_common->non_static_models_init_poses.end())
      ...
      _ingested_model->SetWorldPose(
          convert_to_pose<ignition::math::Pose3d>(it->second));

    _ingestor_common->ingestor_filled =
        false; // Assumes ingestor can only hold 1 object at a time
  }
}
...
void TeleportIngestorPlugin::on_update() {
  ...
  std::function<void(FleetStateIt, std::vector<rmf_plugins_utils::SimEntity> &)>
      fill_robot_list_cb =
  ...
  _ingestor_common->on_update(fill_robot_list_cb, find_nearest_model_cb,
                              get_payload_model_cb, transport_model_cb,
...
void TeleportIngestorPlugin::Load(gazebo::physics::ModelPtr _parent,
  ...
  for (const auto &m : model_list) {
    ...
    if (m && !(m->IsStatic()) && m_name != _model->GetName()) {
      _ingestor_common->non_static_models_init_poses[m_name] =
          convert_pose(m->WorldPose());
    }
  }
...
TeleportIngestorPlugin::~TeleportIngestorPlugin() { rclcpp::shutdown(); }
