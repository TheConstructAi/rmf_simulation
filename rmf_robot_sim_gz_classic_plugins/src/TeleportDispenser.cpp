...
private:
  // Stores params representing state of Dispenser, and handles all message
  // pub/sub
...
SimEntity TeleportDispenserPlugin::find_nearest_model(
    const std::vector<SimEntity> &models, bool &found) const {
...
void TeleportDispenserPlugin::fill_robot_list(
...
void TeleportDispenserPlugin::on_update() {
  ...
  std::function<void(const SimEntity &)> place_on_entity_cb = std::bind(
      &TeleportDispenserPlugin::place_on_entity, this, std::placeholders::_1);
...
TeleportDispenserPlugin::~TeleportDispenserPlugin() { rclcpp::shutdown(); }
