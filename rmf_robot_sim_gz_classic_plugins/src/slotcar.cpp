...
private:
  ...
  get_obstacle_positions(const gazebo::physics::WorldPtr &world);
  ...
  void send_control_signals(const std::pair<double, double> &displacements,
    ...
    auto joint_signals = dataPtr->calculate_joint_control_signals(
        w_tire, displacements, dt, target_linear_speed_now,
        target_linear_speed_destination, max_linear_velocity);
    for (std::size_t i = 0; i < 2; ++i) {
      joints[i]->SetParam("vel", 0, joint_signals[i]);
      joints[i]->SetParam(
          "fmax", 0,
          10000000.0); // TODO(MXG): Replace with realistic torque limit
    }
  }
};
...
SlotcarPlugin::SlotcarPlugin()
  ...
  _charge_state_sub = _gazebo_node->Subscribe(
      "/charge_state", &SlotcarPlugin::charge_state_cb, this);
...
std::vector<Eigen::Vector3d>
SlotcarPlugin::get_obstacle_positions(const gazebo::physics::WorldPtr &world) {
...
void SlotcarPlugin::OnUpdate() {
  ...
  auto update_result = dataPtr->update(rmf_plugins_utils::convert_pose(pose),
